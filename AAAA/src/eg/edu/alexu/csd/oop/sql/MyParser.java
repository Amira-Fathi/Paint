	...
	public int[] selectAfterOperator(String attribute, String query,
		...
		if(indexCondition != -1)
		{
			...
			else //if(dataType.equalsIgnoreCase("varchar"))
			{
				...
				//int comaEnd = query.lastIndexOf("'");
				//operandVal = operandPart.substring(comaStart+1, comaEnd);
				operandVal = operandPart.replaceAll("'", "");
			...
			if(operandVal == null)
			{
				throw  new SQLException(query + "in my parser" +dataType +" "+operandVal  +" "+ indexCondition + "rewan"+tableStr[0].length + "kokok "+tableStr.length+ attribute);
			}
		  ...
		  else
		 {
			...
			for(int m = 0 ; m < tableStr.length;m++)
			{
				if(operator.equals("="))
				{
					if(tableStr[m][indexCondition].equalsIgnoreCase(operandVal))
					 {	selected[m] = 1;
					    this.returnedVal++;
					 }
				}
