...
public class ModelEngine implements DrawingEngine{
	...
	@Override
	// redraw all the current shapes on the canvas
	/* refresh should clear all shapes first ??   */
	...
	public void save(String path){
		try {
			...
			String exten = parts[parts.length-1].toUpperCase();
			parser = (Parser)(Class.forName("parsers."+exten)).newInstance();
			parser.getPath(path);
			parser.write(shapes);
	...
	public void load(String path){
		try {
			...
			String exten = part[part.length-1].toUpperCase();
			parser = (Parser)(Class.forName("parsers."+exten)).newInstance();
			parser.getPath(path);
			shapes = parser.read();
			//h.add(shapes);
